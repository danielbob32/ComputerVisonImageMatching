...
def sequential_ransac(points, num_planes=2, max_trials=2000, residual_threshold=0.0001):
    ...
    for i in range(num_planes):
        ...
        if i!=0:
          or_mask = np.zeros_like(masks[0], dtype=bool)
          for mask in masks:
              or_mask = np.logical_or(or_mask, mask)
          inlier_mask_new = np.zeros(masks[0].shape, dtype=bool)
          j2=0
          for j1 in range(len(masks[0])):
              if or_mask[j1]:
                inlier_mask_new[j1] = False
              else:
                inlier_mask_new[j1] = inlier_mask[j2]
                j2+=1
        else:
          inlier_mask_new = inlier_mask.copy()
        masks.append(inlier_mask_new)
        plane_coef = ransac.estimator_.coef_
        norms.append([plane_coef[1], plane_coef[2], ransac.estimator_.intercept_])
...
def main():
    ...
    F, mask_F = cv2.findFundamentalMat(pts1, pts2, cv2.RANSAC)
    ...
    pts1 = pts1[mask_E.flatten() == 1]
    pts2 = pts2[mask_E.flatten() == 1]
